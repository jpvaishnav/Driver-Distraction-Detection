...
def main():
    ...
    X_data,Y_label,X_pred_general = Load('X_vgg.npy','Y_int_vgg.npy','Y_vgg.npy')
    ...
    model = CNN_fit(X_train,X_test,Y_train,Y_test)
...
def X_Y_data():
    ...
    for n in range(i):
        w = "F:\Trimester2\BTP\BTP\Jan11\\train\c"+str(n)+"\*.jpg"
        print(w)
        e = "c"+str(n)
...
def CNN_fit(X_train,X_test,Y_train,Y_test):
    ...
    plt.show()
    #demarcate these 2 paramteres in seperate graph
    return hist
